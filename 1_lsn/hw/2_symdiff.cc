...
На вход подаётся последовательность чисел в диапазоне от 1 до 20000, разделённых пробелами и заканчивающаяся
нулём — ограничителем последовательности, не входящим в множество. Все эти элементы образуют множество A,
то есть, среди нет одинаковых пар. После разделителя подаются элементы множества B, тоже завершающиеся нулём,
 не входящим в множество.

==================Output format==================

На выход программы нужно вывести симметрическую разность множеств A и B в порядке возрастания элементов.
Элементы выходного множества должны быть разделены пробелами. Для сортировки допустим любой алгоритм.
